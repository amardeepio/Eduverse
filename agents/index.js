...
try {
  ...
  TutorChatAgent.initialize();
  ProgressTrackerAgent.initialize();
  // This sets up the AI model with the API key
...
app.post('/task', async (req, res) => {
    ...
    else if (agentName === 'ProgressTrackerAgent') {
        try {
            const result = await ProgressTrackerAgent.handleTask(task);
            res.json({ success: true, result });
        } catch (error) {
            res.status(500).json({ success: false, error: error.message });
        }
